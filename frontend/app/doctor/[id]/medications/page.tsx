...
export default function MedicationsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Compile detailed medication intelligence
  const medicationGroups = useMemo((): MedicationGroup[] => {
    const groups: Record<string, MedicationGroup> = {};

    // 1. Scan timeline events
    timelineEvents.forEach(ev => {
      ev.medications?.forEach(med => {
        const normalized = med.trim();
        if (!normalized) return;

        // Trace associated diseases from graph links
        ...
        ev.graph_links?.forEach(link => {
          if (
            ...
            // If the medication is the target, associate the source (usually the disease)
            if (link.target.toLowerCase() === normalized.toLowerCase()) {
              linkedDiseases.push(link.source);
            } else if (link.source.toLowerCase() === normalized.toLowerCase()) {
              linkedDiseases.push(link.target);
            }
          }
        });
        ...
        // Update dates
        if (new Date(ev.date) < new Date(groups[normalized].firstPrescribed)) {
          groups[normalized].firstPrescribed = ev.date;
        }
        ...
        // Add associations
        linkedDiseases.forEach(d => {
          if (!groups[normalized].associatedDiseases.includes(d)) {
            groups[normalized].associatedDiseases.push(d);
          }
        });
        doctors.forEach(doc => {
    ...
    documents.forEach(doc => {
      doc.extracted_json?.medications?.forEach(med => {
        const normalized = med.trim();
        if (!normalized) return;
        ...
        if (!groups[normalized]) {
          groups[normalized] = {
            name: normalized,
            occurrences: [],
            firstPrescribed: doc.upload_date.split("T")[0],
        ...
        if (!exists) {
          groups[normalized].occurrences.push({
            ...
            contextText: doc.extracted_json?.diagnoses?.join(", ") || null
          });
        }
      });
    });
  ...
  const filteredGroups = useMemo(() => {
    return medicationGroups.filter(g => 
      g.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
      g.associatedDiseases.some(d => d.toLowerCase().includes(searchQuery.toLowerCase()))
    );
  }, [medicationGroups, searchQuery]);

  if (loading) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center py-24">
        <Loader2 className="w-10 h-10 text-indigo-500 animate-spin mb-4" />
        <p className="text-sm text-slate-400 font-medium">Reconstructing patient medication ledger...</p>
      </div>
    );
  }
  ...
  if (error || !patient) {
    return (
      <div className="flex-1 max-w-4xl w-full mx-auto p-6 flex flex-col justify-center items-center py-20 text-center">
        ...
        <h2 className="text-lg font-bold text-slate-200 mb-2">Failed to Load Medications</h2>
    ...
    <div className="flex-1 w-full max-w-[1440px] mx-auto p-6 flex flex-col gap-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-850 pb-6">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 bg-gradient-to-tr from-indigo-500/10 to-violet-500/10 border border-indigo-500/25 rounded-2xl flex items-center justify-center text-indigo-400">
            <Pill className="w-6 h-6 animate-pulse" />
          </div>
          <div>
            <h1 className="text-xl font-black text-white tracking-tight leading-tight">Patient Medication Ledger</h1>
            <p className="text-xs text-slate-400 mt-1">
              Active and historical medications mapped across all clinical records for <strong className="text-slate-200">{patient.name}</strong>.
            </p>
          </div>
        </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-center justify-between">
        <div className="relative w-full max-w-md">
          ...
          <input
            type="text"
            placeholder="Search medications or linked conditions..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full bg-slate-900/40 border border-slate-800 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 rounded-2xl pl-10 pr-4 py-2.5 text-xs text-white placeholder-slate-500 focus:outline-none transition-all"
          />
        </div>
        <div className="text-xs text-slate-500 font-medium">
          Showing {filteredGroups.length} of {medicationGroups.length} unique medications
        </div>
      </div>
      ...
      {filteredGroups.length === 0 ? (
        <div className="py-20 text-center border border-dashed border-slate-850 rounded-3xl bg-slate-900/10 flex flex-col items-center justify-center">
          ...
          <h3 className="text-sm font-bold text-slate-400">No Medications Found</h3>
          <p className="text-xs text-slate-500 mt-1 max-w-xs">
            {searchQuery ? "Try adjusting your search filters." : "This patient has no recorded medications in their clinical memory graph."}
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          {/* Medications Card List (Left) */}
          <div className="lg:col-span-12 flex flex-col gap-6">
            {filteredGroups.map((group) => (
              <div 
                key={group.name}
                className="bg-slate-900/30 border border-slate-850 hover:border-indigo-500/30 rounded-3xl p-6 backdrop-blur-sm shadow-sm transition-all flex flex-col gap-5 relative overflow-hidden group"
              >
                {/* Background Accent */}
                ...
                <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 border-b border-slate-850/50 pb-4">
                  <div className="flex items-start gap-4">
                    <div className="w-10 h-10 rounded-xl bg-indigo-500/5 border border-indigo-500/20 flex items-center justify-center text-indigo-400 shrink-0">
                      <Pill className="w-5 h-5" />
                    </div>
                    <div>
                      <h3 className="text-base font-extrabold text-white tracking-tight group-hover:text-indigo-400 transition-colors">
                        {group.name}
                      </h3>
                      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-[11px] text-slate-400 mt-1 font-medium">
                        <span className="flex items-center gap-1"><Calendar className="w-3.5 h-3.5 text-slate-500" /> First: {group.firstPrescribed}</span>
                        <span className="text-slate-700">•</span>
                        <span className="flex items-center gap-1"><Clock className="w-3.5 h-3.5 text-slate-500" /> Latest: {group.lastMentioned}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2.5 items-center">
                    {group.associatedDiseases.length > 0 && (
                      <div className="flex items-center gap-1.5">
                        <span className="text-[10px] uppercase tracking-wider text-slate-500 font-bold">Indication:</span>
                        {group.associatedDiseases.map(disease => (
                          <span key={disease} className="px-2.5 py-1 bg-rose-500/10 border border-rose-500/25 rounded-lg text-[10px] font-bold text-rose-400">
                            {disease}
                          </span>
                        ))}
                      </div>
                    ...
                    {group.associatedDoctors.length > 0 && (
                      <div className="flex items-center gap-1.5">
                        <span className="text-[10px] uppercase tracking-wider text-slate-500 font-bold">Doctors:</span>
                ...
                <div>
                  ...
                  <div className="flex flex-col gap-3.5">
                    {group.occurrences.map((occ, idx) => (
                      <div 
                        key={idx} 
                        className="flex gap-4 items-start pl-3 border-l-2 border-slate-800 hover:border-indigo-500/30 transition-colors relative"
                      >
                        {/* Dot indicator */}
                        <div className="w-2 h-2 rounded-full bg-slate-750 absolute -left-[5px] top-1.5 group-hover:bg-indigo-500 transition-colors" />
                        
                        <div className="flex-1 grid grid-cols-1 md:grid-cols-12 gap-3">
                          {/* Date and Event Summary */}
                          <div className="md:col-span-4">
                            <span className="text-[10px] font-bold font-mono text-indigo-400 block">{occ.date}</span>
                          ...
                          <div className="md:col-span-3 flex justify-end items-start">
                            {occ.docId ? (
                              <Link
                                ...
                                className="flex items-center gap-1 text-[10px] font-bold text-indigo-400 hover:text-indigo-300 border border-indigo-500/10 hover:border-indigo-500/30 bg-indigo-500/5 hover:bg-indigo-500/10 px-2.5 py-1.5 rounded-xl transition-all"
                              >
